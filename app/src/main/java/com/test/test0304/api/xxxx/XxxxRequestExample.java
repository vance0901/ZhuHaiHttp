...
import com.test.test0304.TestApplication;
 ...
 * x业xxxxxAPI请求示例
 */
public class XxxxRequestExample {
    private static final String TAG = "xxxxRequest";
    private final Context context;
    private final XxxxService xxxxService;
    
    public XxxxRequestExample(Context context) {
        this.context = context;
        this.xxxxService = XxxxService.getInstance();
    }
...
//        TestApplication.setAuthToken(token);
    ...
    public void setCustomHeaderExample(String headerKey, String headerValue) {
        Log.d(TAG, "设置自定义请求头: " + headerKey + " = " + headerValue);
         ...
         HttpManager.getInstance().addGlobalHeader(headerKey, headerValue);
     ...
     * 清除指定请求头示例
     * 
     * @param headerKey 要清除的请求头名称
     */
    public void clearHeadersExample(String headerKey) {
        Log.d(TAG, "清除请求头: " + headerKey);
        
        // 方式一：使用应用程序类提供的方法清除特定头
//        TestApplication.removeCustomHeader(headerKey);
        
        // 方式二：直接使用HttpManager清除特定头
        // HttpManager.getInstance().removeGlobalHeader(headerKey);

        HttpManager.getInstance().removeGlobalHeader(headerKey);
        
        Toast.makeText(context, "请求头 '" + headerKey + "' 已清除", Toast.LENGTH_SHORT).show();
    }
    ...
    public void loginJsonExample(String phone, String verifyCode) {
        xxxxService.login(phone, verifyCode, new RequestCallback<XxLoginResponse>() {
            @Override
            public void onStart() {
                Log.d(TAG, "开始登录请求");
...
//                        (data != null ? data.getMessage() : ""), Toast.LENGTH_LONG).show();
    ...
    public void updateUserInfoExample(int userId, String userName, String phone, int sex, 
        ...
        xxxxService.updateUserInfo(request, new RequestCallback<UserUpdateResponse>() {
            @Override
            public void onStart() {
    ...
    public void deleteDataExample(String id) {
        // 不再自动添加token header，改为使用全局设置的header
        // HttpManager.getInstance().addGlobalHeader("token", "12345678abcdef");
        // Log.d(TAG, "添加删除请求token header: token=12345678abcdef");
        
        // 打印当前的请求头信息，帮助调试
        Map<String, String> currentHeaders = HttpManager.getInstance().getGlobalHeaders();
        Log.d(TAG, "单个删除请求使用的全局请求头: " + currentHeaders);
        
        xxxxService.deleteData(id, new RequestCallback<DeleteResponse>() {
            @Override
            public void onStart() {
                Log.d(TAG, "开始删除数据请求");
            ...
            public void onSuccess(DeleteResponse data) {
                ...
                Toast.makeText(context, "删除成功: " +
                        (data != null ? data.getMsg() + ", 数据: " + data.getData() : ""),
    ...
    public void batchDeleteDataExample(List<String> ids) {
        ...
        // 打印当前的请求头信息，帮助调试
        Map<String, String> currentHeaders = HttpManager.getInstance().getGlobalHeaders();
        Log.d(TAG, "批量删除请求使用的全局请求头: " + currentHeaders);
        
        xxxxService.deleteData(ids, new RequestCallback<DeleteResponse>() {
            @Override
            public void onStart() {
